...
data['R_Deaths'] = data['N_Deaths'] * (((data['Hazard Fre']/data['total_h']) + (data['Sumsum']/data['total_pop']))/2)
data['R_Damages'] = data['N_D_USD'] * (((data['Hazard Fre']/data['total_h']) + (data['Sumsum']/data['total_pop']))/2)

data_clean = data[['NAME_1_x',"Year",'GID_1_2', 'disaster','began', 'ended','N_Deaths', 'N_D_USD','R_Deaths', 'R_Damages' ]]
data.to_csv(r'LS_Impacts_HRPop.csv')
data_clean.to_csv(r'LS_CLean_Impacts_HRPop.csv')
